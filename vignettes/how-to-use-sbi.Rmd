...
```{r}
# library(sbi) TODO: revive this line
devtools::load_all('~/Research/sbi_code') # TODO: remove this line
```
...
library(pomp)
```
The `pomp`-style definitions of the stochastic volatility model can be found in the file in `R/SV_pomp.R` in the package source file.
...
set.seed(729875)
# to save computation time, simulation log-likelihoods will be loaded from saved data
filename <- paste0("../data/SV/filter_results_sumro=",round(sum(r_o),3),",par=",
    paste(inc,collapse=','),",sim_widths=",
    paste(round(sim_widths[parid],3),collapse=','),",M=",M,",Np=",Np,".rd")
if (file.exists(filename)) {
    load(filename)
} else { # if the saved file doesn't exist, the simulations may be carried out as follows 
